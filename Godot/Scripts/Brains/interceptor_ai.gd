# AI brain that steers toward the closest target in a group. Emits to move for movement, not rotation.
...
func _physics_process(delta: float) -> void:
	...
	parent.move.emit(Vector2.from_angle(current_aim_angle))

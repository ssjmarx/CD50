...
func _ready() -> void:
	...
	# create default circle collision shape from collision_radius
	...
	# cache collision buffer ref
	_collision_buffer = game.collision_buffer

	# configure physics layers from collision matrix
...
func _physics_process(delta: float) -> void:
	...
	# resolve velocity: add first, then set overrides
	velocity += _accumulated_velocity_add
	if _velocity_set_pending != null:
		velocity = _velocity_set_pending

	# resolve angular velocity: same add-then-set pattern
	...
	for i in range(MAX_COLLISION_ITERATIONS):
		...
		# queue collision for buffer flush (not emitted yet)
...
func flush_collisions() -> void:
	for col in _pending_collisions:
		...
		# emit reverse signal on the collider so it knows it was hit
...
func _complete_deactivation() -> void:
	...
	# notify components to disconnect signals and reset state
...
func activate() -> void:
	...
	# notify components to reconnect signals
...
func _find_collision_handler(collider) -> Callable:
	...
	# second pass: catch-all handlers (layers == 0)
	for entry in _collision_handlers:
		if entry["layers"] == 0:
			return entry["handler"]

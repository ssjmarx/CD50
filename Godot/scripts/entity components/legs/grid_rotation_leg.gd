...
var _tetromino_guts  # sibling TetrominoGuts, typed loosely to avoid hard dep
...
func _get_cell_size() -> Vector2:
	# read from sibling GridMovementLeg if present, else default
	for child in entity.get_children():

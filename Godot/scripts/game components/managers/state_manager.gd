...
func initialize() -> void:
	for t in transitions:
		if t.is_valid():
			...
			push_warning("StateManager: skipping invalid transition (empty group names).")
...
func _process_trigger(t: CDTransition) -> void:
	...
	## selector narrows the pool (receives manager position for distance-based selectors)

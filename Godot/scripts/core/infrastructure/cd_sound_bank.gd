...
func _process(_delta: float) -> void:
	...
	for voice in _voices:
		...
		# advance to next note in jingle or end the voice
	...
	# disable processing when all voices are idle
	if not any_active:
		_has_active = false
		set_process(false)
...
func play_one_shot(def: CDSoundDef, sound_position: Vector2, positional: bool,
	...
	# start playback and grab the stream buffer
...
func start_continuous(signature: String, wave_shape: int, effect: int,
	...
	if _continuous_registry.has(signature):
		...
		# switch to global position when multiple sources share one voice

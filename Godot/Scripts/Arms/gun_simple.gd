# Classic arcade-style gun. Limits active bullets to prevent spamming.
# Supports joystick and mouse aim.

extends UniversalComponent

signal target_hit(target: Node2D)

# Ammo and firing configuration
@export var ammo: PackedScene 
@export var max_bullets: int = 4
@export var muzzle_offset: int = 20
@export var initial_velocity: int = 800

# Aim input mode (enable one or both)
...
# Connect shoot and aim signals from the parent body
func _ready() -> void:
...
# Spawn a bullet if under the active limit, using current aim or parent rotation
func _on_shoot() -> void:
	...
	var firing_angle: float = aim_angle if has_aim_input else parent.rotation	

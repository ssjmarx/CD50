## centralized audio engine for V2
...
var _continuous_registry: Dictionary = {}  # signature : Voicee
...
func play_one_shot(def: CDSoundDef, sound_position: Vector2, positional: bool,
	...
	# skip if this caller already has an active voice
	if exclusive:
		for v in _voices:
			if v.active and v.source_id == caller_id:
				return false
	
	# check if this caller already has an active voice
	...
	# no active voice for this caller
	if voice == null:
...
## advance to next note in a jingle
...
func start_continuous(signature: String, wave_shape: int, effect: int,
	...
	if _continuous_registry.has(signature):
		...
		# use global position for duplicated continuous sounds
...
## inner class for audio stream generators
class Voice:
	var player: Node

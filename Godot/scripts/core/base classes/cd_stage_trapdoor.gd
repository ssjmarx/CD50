## cd_stage_trapdoor.gd
## Produces: spawned entities (fresh or pooled) into the game tree, staggered per wave.
## Consumes: trigger/safe/unsafe bus signals; game.blackboard wave key; spawn scenes/pool.
...
## Receive trigger signal and enter delay phase before spawning.
...
## Override to populate _spawn_queue for a given wave (default fills 0..count-1).
...
## Mark the spawn zone safe (stagger loop may proceed).
func _on_zone_safe() -> void:
	_zone_is_safe = true

## Mark the spawn zone unsafe (stagger loop pauses).

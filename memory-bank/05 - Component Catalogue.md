...
| Script | Extends | Summary |
|--------|---------|---------|
| `ball.gd` | `extends UniversalBody` | Pong ball. Draws a colored square and sets up collision shapes from a radius export. |
| `paddle.gd` | `extends UniversalBody` | Pong paddle. Draws a white rectangle sized from width/height exports. |
...
| `brick.gd` | `extends UniversalBody` | Breakout brick with health-based coloring. Color shifts from green to red as HP decreases. |
| `barrier.gd` | `extends UniversalComponent2D` | Space Invaders-style barrier. Made from around thirty bricks that individually take damage and die. |
...
| `triangle_ship.gd` | `extends UniversalBody` | Asteroids player ship. Draws a triangular outline with a notched tail. |
| `ufo.gd` | `extends UniversalBody` | Space Invaders-style UFO. Scales speed and accuracy for the SMALL variant. Draws a three-layer saucer shape. |
...
| `falling_ai.gd` | `extends UniversalComponent` | AI brain that emits a move signal at regular intervals, used for Tetris-style falling movement. |
...
| `engine_simple.gd` | `extends UniversalComponent` | Simple engine for Asteroids-style thrust. Accelerates forward, caps at top speed. |
...
| `pong_acceleration.gd` | `extends UniversalComponent` | Pong-style acceleration. Ramps velocity through discrete levels on paddle collision. |
...
| `screen_wrap.gd` | `extends UniversalComponent` | Asteroids-style screen wrapping. Warps parent to opposite side when off-screen. |
